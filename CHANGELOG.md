...
**Файлы:**
- `migrations/028_timeline_fill_gaps_with_zeros.sql` - новая версия функции
- `scripts/tmp/compare-timeline-before-after.cjs` - проверка результата

**Как применить:**
```
1. Открой Supabase SQL Editor
2. Запусти migrations/028_timeline_fill_gaps_with_zeros.sql
3. Проверь: node scripts/tmp/compare-timeline-before-after.cjs
   Должно показать: "✅ МИГРАЦИЯ ПРИМЕНЕНА! - Ровно 30 точек"
```
