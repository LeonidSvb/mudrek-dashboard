...
### Проблема:
- **MCP Supabase инструменты не загружаются в Claude Code**
- Конфигурация `.mcp.json` корректна (токен валиден, формат правильный)
- Токен `sbp_10b4ce3e77c2fb770870a579e64536a15c90fb79` синхронизирован с `.env`
- Ожидаемые инструменты отсутствуют:
  - `mcp__supabase__query`
  - `mcp__supabase__insert`
  - `mcp__supabase__update`
  - `mcp__supabase__delete`
- Видны только `mcp__ide__*` инструменты

### Диагностика:

**Проверенные файлы:**
- ✅ `.mcp.json` - конфигурация правильная
- ✅ `.env` - токен `SUPABASE_ACCESS_TOKEN` совпадает
- ✅ `docs/MCP_SETUP.md` - документация актуальна
...
```json
{
  "mcpServers": {
    "supabase": {
      ...
      "env": {
        "SUPABASE_ACCESS_TOKEN": "sbp_10b4ce3e77c2fb770870a579e64536a15c90fb79"
      }
    }
  }
}
...
1. **VS Code не был перезапущен после создания `.mcp.json`**
   - MCP серверы загружаются только при старте Claude Code
   - Требуется полный перезапуск VS Code

2. **Проблема с загрузкой npm пакета:**
   - Пакет `@supabase/mcp-server-supabase` не установлен
   - Может требоваться ручная установка через `npx`

3. **Конфликт с глобальной конфигурацией:**
   - Возможно, глобальный `claude_desktop_config.json` блокирует локальный
   - Требуется проверка приоритета конфигураций

### Следующие шаги:

1. **Перезапуск VS Code** (пользователь выполняет)
2. Проверка доступности `mcp__supabase__*` инструментов после перезапуска
3. Если не помогло - проверка логов MCP в `~/.claude/logs/`
4. Ручная установка пакета: `npx -y @supabase/mcp-server-supabase@latest`

### Статус:
⏳ **В ОЖИДАНИИ** - Требуется перезапуск VS Code для активации MCP серверов

---

## [v3.35.0] - 2025-10-30 - Локальная конфигурация MCP для мультиаккаунтной поддержки

### Проблема:
- **Глобальный MCP конфиг не работает с 2 Supabase аккаунтами**
- Токены конфликтовали между проектами (невозможно иметь разные токены)
- GitHub MCP токен истек (25 октября 2025) и не обновлялся
- Конфигурация не отслеживалась через Git
- При переключении между проектами MCP ломался

### Решение - Локальная MCP конфигурация:

#### Структура:
```
.mcp.json              # Локальная конфигурация (в .gitignore, с токенами)
.mcp.json.example      # Template для Git (без токенов)
docs/MCP_SETUP.md      # Полная инструкция по настройке
```

#### Изменения конфигурации:

**Локальный `.mcp.json` (для этого проекта):**
- ✅ Supabase MCP - доступ к БД (токен: `sbp_10b4...`)
- ✅ Filesystem MCP - доступ к файлам проекта
- ✅ GitHub MCP - работа с репозиторием (новый токен: `ghp_8LvT...`, без срока действия)

**Глобальный `claude_desktop_config.json` (очищен):**
- ❌ Удален Supabase MCP (перемещен в локальный)
- ❌ Удален GitHub MCP (перемещен в локальный)
- ✅ Остались: sequential-thinking, shadcn, neon, n8n (универсальные)

#### Преимущества:

1. **Мультиаккаунтная поддержка:**
   - Project 1: Supabase Account A + GitHub Account A
   - Project 2: Supabase Account B + GitHub Account B
   - Автоматическое переключение при смене проекта

2. **Git Tracking:**
   - `.mcp.json.example` в репозитории
   - Коллеги могут скопировать и настроить за 1 минуту
   - История изменений конфигурации

3. **Изоляция:**
   - Изменения в одном проекте не влияют на другой
   - Легко восстановить при поломке

4. **Безопасность:**
   - `.mcp.json` в `.gitignore` (токены не попадают в Git)
   - Создан backup глобального конфига

### Файлы:

**Добавлено:**
- `.mcp.json.example` - template для настройки MCP
- `docs/MCP_SETUP.md` - полная инструкция (Quick Start, Troubleshooting, История)

**Обновлено:**
- `.mcp.json` - добавлен GitHub MCP с новым токеном
- `.env` - синхронизирован токен Supabase
- Global config - очищен от проект-специфичных серверов

### Коммиты:
- `2a0c5c5` - docs: migrate to local MCP configuration for multi-account support
- `ef21e33` - refactor: daily log rotation + project cleanup (включает GitHub MCP)

### Статус:
✅ **ГОТОВО** - Локальная конфигурация работает, все токены валидны, документация создана
