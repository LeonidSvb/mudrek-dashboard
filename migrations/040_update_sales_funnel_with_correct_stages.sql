-- Migration 040: Update get_sales_funnel_metrics() with correct stage IDs
-- Purpose: Fix deal stages to match actual HubSpot pipeline
-- Created: 2025-10-17
...
DECLARE
  ...
  v_deals_qualified_to_buy INTEGER;
...
BEGIN
  ...
  -- CORRECTED STAGE IDS:
  -- appointmentscheduled = "Qualified to Buy"
  -- qualifiedtobuy = "High Interest / Offer Sent"
  -- closedwon = "Closed Won"
  -- closedlost = "Closed Lost"
  SELECT
    COUNT(*) FILTER (WHERE dealstage = 'appointmentscheduled'),
    COUNT(*) FILTER (WHERE dealstage = 'qualifiedtobuy'),
    COUNT(*) FILTER (WHERE dealstage = 'closedwon'),
    COUNT(*) FILTER (WHERE dealstage = 'closedlost')
  INTO
    v_deals_qualified_to_buy,
  ...
  SELECT json_build_object(
    ...
    'deals', json_build_object(
      ...
      'qualified_to_buy', COALESCE(v_deals_qualified_to_buy, 0),
...
'Sales funnel metrics with correct HubSpot stage IDs: appointmentscheduled, qualifiedtobuy, closedwon, closedlost';
